...
docker compose exec -T web yarn typecheck --if-present
...
| `Cannot find module 'react' or its corresponding type declarations` | Missing types | `docker compose exec -T web yarn add -D @types/react @types/react-dom` |
...
| Error | Fix |
|---|---|
| `Invalid hook call. Hooks can only be called inside of the body of a function component` | Multiple React copies in `node_modules`. Run `docker compose exec -T web yarn why react` — should show exactly one. Use `resolutions`/`overrides` in `package.json` to dedupe. |
...
```bash
docker compose exec -T web yarn why react                       # check for duplicates
docker compose exec -T web yarn why @types/react                # check version alignment
docker compose exec -T web yarn install                         # consolidate duplicates
# Only when `docker compose exec -T web yarn why react` reports duplicates or a version mismatch with `@types/react`.

# docker compose exec -T web yarn add react@^<major> react-dom@^<major>
```
